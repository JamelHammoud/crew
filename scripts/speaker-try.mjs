...
const TRIES = [
  ...
  ['cone + in wave + slash', [CONE, WAVE_IN, SLASH]],
  ['cone + out wave + slash', [CONE, WAVE_OUT, SLASH]],
  ['cone + X 5 gap 3', [CONE, 'm16.75 9.5 5 5M21.75 9.5l-5 5']],
  ['cone + X 4.5 right', [CONE, 'm17.25 9.75 4.5 4.5M21.75 9.75l-4.5 4.5']],
  ['bare cone', [CONE]],
  ['cone + short slash', [CONE, 'm6 6 12 12']],
  ['cone right + slash', ['M6.5 9.5h3.25L15 5.25a.75.75 0 0 1 1.25.6v12.3a.75.75 0 0 1-1.25.6L9.75 14.5H6.5a1.25 1.25 0 0 1-1.25-1.25v-2.5A1.25 1.25 0 0 1 6.5 9.5Z', SLASH]]
]
...
const card = ([name, paths]) => `
  <div class="card">
    <div class="row"><span class="s16">${svg(paths)}</span><span class="s16 big">${svg(paths)}</span></div>
...
await writeFile(
  ...
  `<!doctype html><meta charset="utf8"><style>
    ...
    .card{background:#151719;border-radius:14px;padding:12px;display:flex;flex-direction:column;align-items:center;gap:10px}
    .row{display:flex;align-items:center;gap:16px;height:64px}
