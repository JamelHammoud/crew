...
app.whenReady().then(async () => {
  ...
  try {
    ...
    seen.quoteCollapsed = await win.webContents.executeJavaScript("(() => { const frame = document.querySelector('iframe[title=\\\"Message\\\"]'); const show = [...document.querySelectorAll('button')].find(node => node.textContent.trim() === 'Show earlier mail'); const old = frame && frame.contentDocument.querySelector('#old-reply'); const quoted = old && old.closest('[data-crew-old-mail]'); return Boolean(frame && show && quoted && getComputedStyle(quoted).display === 'none') })()")
    ...
    await wait(260)
    seen.quoteClosed = await win.webContents.executeJavaScript("(() => { const frame = document.querySelector('iframe[title=\\\"Message\\\"]'); const show = [...document.querySelectorAll('button')].find(node => node.textContent.trim() === 'Show earlier mail'); const old = frame.contentDocument.querySelector('#old-reply'); const quoted = old.closest('[data-crew-old-mail]'); const gap = show.getBoundingClientRect().top - frame.getBoundingClientRect().bottom; return { height: parseFloat(frame.style.height), gap, hidden: getComputedStyle(quoted).display === 'none' } })()")
    ...
    win.webContents.sendInputEvent({ type: 'keyUp', keyCode: 'Tab' })
    await wait(80)
    seen.composeMessageTab = await win.webContents.executeJavaScript("document.activeElement?.getAttribute('aria-label')")
    await shoot(win, 'compose')
    seen.formattingClosed = await win.webContents.executeJavaScript("!document.querySelector('[role=toolbar][aria-label=\"Formatting\"]')")
    await win.webContents.executeJavaScript("document.querySelector('button[aria-label=\"Formatting options\"]').click()")
    await wait(180)
    seen.formattingControls = await win.webContents.executeJavaScript("document.querySelectorAll('[role=toolbar][aria-label=\"Formatting\"] button').length")
    seen.formattingAboveSend = await win.webContents.executeJavaScript("(() => { const toolbar = document.querySelector('[role=toolbar][aria-label=\"Formatting\"]'); const send = [...document.querySelectorAll('button')].find(node => node.textContent.trim() === 'Send'); return toolbar && send && toolbar.getBoundingClientRect().bottom <= send.getBoundingClientRect().top })()")
    await shoot(win, 'compose-formatting')
...
try {
  ...
  if (seen.composeStart !== 'To' || seen.composeTab !== 'Subject' || seen.composeMessageTab !== 'Message') throw new Error('the composer tabbed through ' + seen.composeStart + ' → ' + seen.composeTab + ' → ' + seen.composeMessageTab)
  console.log('compose focus     ' + seen.composeStart + ' → ' + seen.composeTab + ' → ' + seen.composeMessageTab)
  if (!seen.formattingClosed || seen.formattingControls < 10 || !seen.formattingAboveSend) throw new Error(`the formatting row reported closed=${seen.formattingClosed}, controls=${seen.formattingControls}, above=${seen.formattingAboveSend}`)
  ...
  console.log(`formatting        ${seen.formattingControls} controls above Send`)
