...
app.whenReady().then(async () => {
  ...
  try {
    ...
    seen.quoteCollapsed = await win.webContents.executeJavaScript("(() => { const frame = document.querySelector('iframe[title=\\\"Message\\\"]'); const show = [...document.querySelectorAll('button')].find(node => node.textContent.trim() === 'Show earlier mail'); const old = frame && frame.contentDocument.querySelector('#old-reply'); return Boolean(frame && show && old && getComputedStyle(old).display === 'none') })()")
    ...
    await wait(260)
    seen.quoteClosed = await win.webContents.executeJavaScript("(() => { const frame = document.querySelector('iframe[title=\\\"Message\\\"]'); const show = [...document.querySelectorAll('button')].find(node => node.textContent.trim() === 'Show earlier mail'); const old = frame.contentDocument.querySelector('#old-reply'); const gap = show.getBoundingClientRect().top - frame.getBoundingClientRect().bottom; return { height: parseFloat(frame.style.height), gap, hidden: getComputedStyle(old).display === 'none' } })()")
