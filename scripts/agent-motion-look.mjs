...
function Morph() {
  ...
  useEffect(() => {
    const timer = setInterval(() => setIndex(value => (value + 1) % activities.length), 2600)
  ...
  return (
    <section className="rounded-card border border-ink-700 bg-ink-800/70 px-8 py-7 flex items-center gap-7">
      <AgentIcon seed="motion-morph" px={112} activity={activity} />
      <div>
        <div className="text-lg font-semibold text-fg">State morph</div>
