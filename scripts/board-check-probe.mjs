...
const driveSource = String.raw`(async () => {
  ...
  await attempt('hovering marks a shape', async () => {
    ...
    return { ok: hovered === shape.id, note: 'hovered ' + hovered + ' path=' + editor.root.getCurrent().getPath() + ' editing=' + editor.getEditingShapeId() + ' culled=' + editor.getCulledShapes().size + ' boxCulled=' + editor.getCulledShapes().has(shape.id) + ' at=' + JSON.stringify(centre) + ' hit=' + String(editor.getShapeAtPoint(editor.inputs.getCurrentPagePoint(), { margin: 8 / editor.getZoomLevel(), hitInside: true, renderingOnly: true })?.id) + ' pagept=' + JSON.stringify(editor.inputs.getCurrentPagePoint()) + ' want=' + JSON.stringify(boundsOf(shape).center) + ' live=' + JSON.stringify(editor.getShape(shape.id) ? { parent: editor.getShape(shape.id).parentId.slice(0,10), hidden: editor.isShapeHidden(shape.id), mask: JSON.stringify(editor.getShapeMask(shape.id)), geo: editor.getShapeGeometry(shape.id).constructor.name, b: editor.getShapeGeometry(shape.id).bounds.toJson(), atPoint: editor.getShapesAtPoint(editor.inputs.getCurrentPagePoint(), { hitInside: true }).map(s => s.type), type: editor.getShape(shape.id).type, pb: editor.getShapePageBounds(shape.id).toJson(), local: editor.getPointInShapeSpace(shape.id, editor.inputs.getCurrentPagePoint()), hitsInside: editor.getShapeGeometry(shape.id).hitTestPoint(editor.getPointInShapeSpace(shape.id, editor.inputs.getCurrentPagePoint()), 0, true), dist: editor.getShapeGeometry(shape.id).distanceToPoint(editor.getPointInShapeSpace(shape.id, editor.inputs.getCurrentPagePoint()), true), filled: editor.getShapeGeometry(shape.id).isFilled, closed: editor.getShapeGeometry(shape.id).isClosed, kids: (editor.getShapeGeometry(shape.id).children || []).map(c => c.constructor.name + ':' + c.isFilled), inPage: editor.getCurrentPageShapesSorted().some(s => s.id === shape.id) } : 'gone') }
  })
