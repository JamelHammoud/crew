...
if (seen.drag.cards !== 0 || seen.drag.line !== '1' || !seen.drag.hand) throw new Error(`the drag was ${JSON.stringify(seen.drag)}`)
if (JSON.stringify(seen.moves) !== JSON.stringify([['one', 2]])) throw new Error(`the move was ${JSON.stringify(seen.moves)}`)
if (JSON.stringify(seen.menu) !== JSON.stringify(['Edit in composer', 'Send now', 'Remove from queue'])) throw new Error(`the menu was ${JSON.stringify(seen.menu)}`)
