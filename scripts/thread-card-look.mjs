...
const READ = \`(() => {
  ...
  return cards.map((card, index) => {
    ...
    return {
      ...
      corners: band ? (() => { const pill = band.firstElementChild; const style = getComputedStyle(pill); const r = pill.getBoundingClientRect(); return { tl: style.borderTopLeftRadius, tr: style.borderTopRightRadius, br: style.borderBottomRightRadius, bl: style.borderBottomLeftRadius, x: Math.round(r.left), y: Math.round(r.top), h: Math.round(r.height), w: Math.round(r.width) } })() : null,
