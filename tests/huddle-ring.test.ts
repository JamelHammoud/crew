...
describe('the noise a huddle makes when it starts', () => {
  ...
  it('gives up on its own rather than ringing until someone answers', () => {
    ...
    arrives(emptyRoom())
    arrives(room(peer('a', 'Ali')))
    vi.advanceTimersByTime(ringLength(CALL) * 1000)
    heard()
  ...
  it('says every turn at the size it said the first, rather than trailing away', () => {
    playRing({ phrase: [{ hz: 600, at: 0, length: 0.1, gain: 1 }], every: 1, times: 3 })
    const first = loudest()
    vi.advanceTimersByTime(1000)
    const second = loudest()
    vi.advanceTimersByTime(1000)

    expect(first).toBeGreaterThan(0)
    expect(second).toBe(first)
    expect(loudest()).toBe(first)
  })
