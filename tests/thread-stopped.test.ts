...
import { buildThread, threadState, THREAD_STATE_LABELS } from '../src/renderer/src/components/thread'
  ...
  it('reads on its card without a word or a mark in danger', () => {
    ...
    render(
      createElement(ThreadCard, {
        thread,
        ts: 1,
        state: threadState(thread, [started, stopped], false),
        detail: '',
        onOpen: () => {}
      })
    )
  ...
  it('still draws a real failure in danger, so the color keeps meaning something', () => {
    ...
    render(
      createElement(ThreadCard, {
        thread,
        ts: 1,
        state: threadState(thread, [started, broke], false),
        detail: '',
        onOpen: () => {}
      })
    )
