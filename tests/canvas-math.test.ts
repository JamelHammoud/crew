...
describe('the identities every vector operation rests on', () => {
  it('holds across four hundred random pairs', () => {
    for (let run = 0; run < 400; run++) {
      ...
      expect(Vec.Sub(Vec.Add(a, b), b).toFixed()).toEqual(a.clone().toFixed())
      ...
      expect(Vec.Lrp(a, b, 1).toFixed()).toEqual(b.clone().toFixed())
      expect(Vec.Med(a, b).toFixed()).toEqual(Vec.Lrp(a, b, 0.5).toFixed())
