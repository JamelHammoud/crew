...
describe('an agent face', () => {
  ...
  it('keeps Reading as two rounded pages with a clear gutter and motion that begins blank', () => {
    ...
    expect(object.querySelector('[data-part="book-page-left-body"]')?.getAttribute('d')).toContain('46 78')
    expect(object.querySelector('[data-part="book-page-right-body"]')?.getAttribute('d')).toContain('54 78')
    expect(object.querySelectorAll('[data-part^="book-line"]')).toHaveLength(4)
    expect(
      Array.from(object.querySelectorAll('[data-part^="book-line"]')).every(
    ...
    expect(styles.split('@keyframes agent-book-page-right')[1]?.split('@keyframes')[0]).not.toContain('rotate')
    expect(styles).not.toContain('@keyframes agent-book-body')
    expect(styles).not.toContain('@keyframes agent-page-turn')
  })

  it('keeps Searching as one connected rounded magnifier with a compact handle', () => {
    ...
    expect(object.querySelector('[data-part="search-handle"]')?.getAttribute('d')).toBe('M56 56 L77 77')
    ...
    expect(object.querySelector('[data-part="search-ring"]')?.getAttribute('r')).toBe('26.5')
    expect(object.querySelector('[data-part="search-hole"]')?.getAttribute('r')).toBe('15.5')
    expect(object.querySelector('[data-part="search-glint"]')).toBeNull()
  ...
  it('crossfades activity states through overlapping blurred shapes', () => {
    ...
    expect(styles).toContain('filter: blur(var(--agent-transition-blur))')
