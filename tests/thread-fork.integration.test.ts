...
const soFar = (text: string | undefined): string =>
  (text ?? "").slice((text ?? "").indexOf("Thread so far:"));
...
describe("what a thread offers to fork with", () => {
  ...
  it("is a thread’s own, so the chat never offers it", () => {
    expect(commandsIn("chat").map((command) => command.name)).not.toContain(
      "fork",
    );
  });
...
describe("forking a thread", () => {
  ...
  async function connectRunner(
    ...
    const runner = testRunner({
      ...
      providers: [
        steerable ? makeSteerableProvider(env) : makeFakeProvider(env),
      ],
  ...
  async function threadWith(
    ...
    const thread = (await ui.waitForEvent(
      (e) => e.kind === "thread.started",
    )) as Started;
    await ui.waitForEvent(
      (e) => e.kind === "agent.end" && e.threadId === thread.threadId,
    );
    return thread;
  }
  ...
  async function forked(
    ...
    const seen = new Set(
      ui.events
        .filter((e) => e.kind === "thread.started")
        .map((e) => e.threadId),
    );
    fork(ui, text, threadId, mentions, forkId);
    return (await ui.waitForEvent(
      (e) =>
        e.kind === "thread.started" &&
        e.forkedFrom === threadId &&
        !seen.has(e.threadId),
    )) as Started;
  }
  ...
  it("opens a thread of its own and leaves the one it came from exactly as it was", async () => {
    ...
    const carried = await forked(
      sam,
      "try the changelog instead",
      thread.threadId,
    );
    ...
    const parent = sam.events.filter(
      (e) => "threadId" in e && e.threadId === thread.threadId,
    );
    expect(
      parent.some(
        (e) => e.kind === "message" && e.text === "try the changelog instead",
      ),
    ).toBe(false);
    ...
    expect(
      sam.messages
        .slice(said)
        .some(
          (m) => m.type === "queue.state" && m.threadId === thread.threadId,
        ),
    ).toBe(false);
  });
  ...
  it("reads what was said before it, in one run of talk, and none of what was said after", async () => {
    ...
    const thread = await threadWith(sam, "@Fake the readme is stale", [
      samsFake,
    ]);
    const carried = await forked(
      sam,
      "carry on with the changelog",
      thread.threadId,
    );
    const first = (await sam.waitForEvent(
      (e) => e.kind === "agent.end" && e.threadId === carried.threadId,
    )) as Ended;
    ...
    expect(block.indexOf("the readme is stale")).toBeLessThan(
      block.indexOf("carry on with the changelog"),
    );
  ...
  it("takes the agent it is handed, and the thread’s own where none is", async () => {
    ...
    await waitUntil(
      () => sam.events.filter((e) => e.kind === "agent.online").length === 2,
    );

    const thread = await threadWith(sam, "@Fake tidy the readme", [samsFake]);
    const carried = await forked(sam, "try it with @Steery", thread.threadId, [
      steery,
    ]);
    ...
    expect(
      sam.events.some(
        (e) => e.kind === "thread.agent" && e.threadId === thread.threadId,
      ),
    ).toBe(false);
  ...
  it("carries the plan, the board and the tickets in with it", async () => {
    ...
    await sam.waitFor(
      (m) =>
        m.type === "design.boards" &&
        m.boards.some((b) => b.id === "the-board"),
    );
    ...
    const thread = (await sam.waitForEvent(
      (e) => e.kind === "thread.started",
    )) as Started;
  ...
  it("carries the plan the thread agreed on once it is being built", async () => {
    ...
    const thread = await threadWith(
      sam,
      "@Fake work out the rewrite",
      [samsFake],
      ["plan"],
    );
    await sam.waitForEvent(
      (e) => e.kind === "thread.plan" && e.threadId === thread.threadId,
    );
    sam.send({ type: "plan.implement", threadId: thread.threadId });
    await sam.waitForEvent(
      (e) => e.kind === "thread.implement" && e.threadId === thread.threadId,
    );
  ...
  it("opens under the id it was asked for, and names one itself where it cannot", async () => {
    ...
    await sam.waitForEvent(
      (e) => e.kind === "agent.end" && e.threadId === mine.threadId,
    );

    const junk = await forked(
      sam,
      "and another way",
      thread.threadId,
      [],
    ...
    await sam.waitForEvent(
      (e) => e.kind === "agent.end" && e.threadId === junk.threadId,
    );
  ...
  it("says why a fork with nothing to carry on with did not happen, to the one who asked", async () => {
    ...
    const notice = (await sam.waitFor(
      (msg) => msg.type === "notice",
    )) as Notice;
    ...
    expect(
      sam.events.some(
        (e) => e.kind === "thread.started" && e.forkedFrom !== undefined,
      ),
    ).toBe(false);
  ...
  it("says why a fork nobody can take did not happen", async () => {
    ...
    await sam.waitFor(
      (m) => m.type === "agent.removed" && m.agentId === patsFake,
    );
    ...
    expect(
      sam.events.some(
        (e) => e.kind === "thread.started" && e.forkedFrom === thread.threadId,
      ),
    ).toBe(false);
  });
  ...
  it("a fork of a hidden thread is hidden, so nobody else sees it and nothing is written down", async () => {
    ...
    for (const other of [samSecondWindow, pat]) {
      expect(
        other.events.some(
          (e) => "threadId" in e && e.threadId === carried.threadId,
        ),
      ).toBe(false);
      expect(
        other.steps.some((step) => step.threadId === carried.threadId),
      ).toBe(false);
    }
    ...
    expect(
      revived.events.some(
        (e) => e.kind === "message" && e.text === "carry on with the changelog",
      ),
    ).toBe(false);
  });
  ...
  it("reads the whole line it came down, and none of what any of them did after", async () => {
    ...
    await sam.waitForEvent(
      (e) => e.kind === "agent.end" && e.threadId === beta.threadId,
    );
    ...
    await sam.waitForEvent(
      (e) => e.kind === "agent.end" && e.threadId === gamma.threadId,
    );
