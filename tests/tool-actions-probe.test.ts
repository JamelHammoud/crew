...
import { STROKE } from '../src/renderer/src/icons/keylines'
...
describe('step rows', () => {
  ...
  it('opens an edit into the lines that went and the lines that arrived', async () => {
    ...
    fireEvent.click(container.querySelector('button') as HTMLButtonElement)
    await waitFor(() => expect(container.textContent).toContain('const one = 1'))
    expect(container.textContent).toContain('const one = 2')
    expect(screen.getByText('−')).not.toBeNull()
    expect(screen.getByText('+')).not.toBeNull()
