...
// The bar hangs under the middle of what it is asking about, and never leaves
// the stage. A selection taller than the viewport, or standing at the bottom of
// it, has its own bottom edge off screen, and a bar pinned to that edge is drawn
...
export function askAnchor(target: Box | null, stage: Size, bar: Size): { x: number; y: number } {
  ...
  const middle = Math.round(target.x + target.w / 2 - bar.w / 2)
  return { x: Math.min(Math.max(middle, EDGE), right), y: Math.min(Math.max(y, EDGE), floor) }
}
