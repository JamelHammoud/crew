...
export default function Inspector({ shape }: { shape: DesignNodeShape }) {
  ...
  const setEffect = (at: number, next: Effect) =>
    patch({ effects: props.effects.map((effect, i) => (i === at ? next : effect)) })
    ...
    <>
          ...
          <>
            <Row>
              <Choice
                ...
                options={DIRECTIONS}
                onPick={value => setLayout({ direction: value })}
              />
      ...
      <Section
        ...
        action={
          <AddButton
            label="Add fill"
            onClick={() => patch({ fills: [...props.fills, { type: 'solid', color: NEW_FILL, opacity: 1, visible: true }] })}
          />
        }
      >
        {props.fills.map((fill, at) => (
          <div key={at} className="flex items-center gap-0.5 -mr-1.5">
            ...
            <ShowButton shown={fill.visible} onClick={() => setFill(at, { ...fill, visible: !fill.visible })} />
          ...
          <AddButton
            ...
            onClick={() =>
              patch({
                strokes: [
                  ...props.strokes,
                  { color: NEW_STROKE, weight: 1, align: 'inside', style: 'solid', visible: true }
                ]
              })
            }
          />
          ...
          <div key={at} className="flex flex-col gap-2">
            ...
            <Row>
              ...
              <NumberInput
                ...
                min={0}
                max={200}
                onChange={weight => setStroke(at, { weight })}
              />
            </Row>
            <Choice
              value={stroke.style}
              options={STROKE_STYLES}
              onPick={value => setStroke(at, { style: value as Stroke['style'] })}
            />
          </div>
        ))}
      </Section>
      ...
      {props.text !== '' && (
        <Section title="Text">
          <Row>
            <NumberInput
              label="Size"
              value={props.type.size}
              min={1}
              onChange={size => patch({ type: { ...props.type, size } })}
            />
            <NumberInput
              label="Weight"
