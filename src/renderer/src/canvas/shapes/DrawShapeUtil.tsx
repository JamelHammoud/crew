...
export class DrawShapeUtil extends ShapeUtil<DrawShape> {
  ...
  getDefaultProps(): DrawShape['props'] { return { segments: [], color: 'black', fill: 'none', dash: 'draw', size: 'm', isComplete: false, isClosed: false, isPen: false, scale: 1, scaleX: 1, scaleY: 1 } }
  ...
  getGeometry(shape: DrawShape): Geometry2d {
    ...
    if (shape.props.isClosed && centerline.length > 2) return new Polygon2d({ points: centerline, isFilled: shape.props.fill !== 'none' })
    return centerline.length > 1 ? new Polyline2d({ points: centerline }) : new Circle2d({ x: -width, y: -width, radius: width, isFilled: true })
  }
  ...
  component(shape: DrawShape): ReactNode {
    ...
    const fill = shape.props.fill === 'none' ? 'none' : shape.props.fill === 'semi' ? canvasSurface(this.editor) : shapeColor(this.editor, shape.props.color, shape.props.fill === 'solid' ? 'semi' : shape.props.fill === 'lined-fill' ? 'linedFill' : shape.props.fill)
    const dash = shape.props.dash === 'dashed' ? `${width * 2} ${width * 2}` : shape.props.dash === 'dotted' ? `0 ${width * 2}` : undefined
    if (points.length < 2) return createElement('svg', { width: '100%', height: '100%', style: { overflow: 'visible' } }, createElement('circle', { cx: 0, cy: 0, r: width / 2, fill: stroke }))
    const body = shape.props.dash === 'draw'
      ? createElement('path', { d: pathFromPoints(outline, true), fill: stroke })
      : createElement('path', { d: pathFromPoints(freehandCenterline(points, options), shape.props.isClosed), fill: 'none', stroke, strokeWidth: width, strokeDasharray: dash, strokeLinecap: 'round', strokeLinejoin: 'round' })
    const closedFill = shape.props.isClosed && fill !== 'none' ? createElement('path', { d: pathFromPoints(freehandCenterline(points, options), true), fill }) : null
    return createElement('svg', { width: '100%', height: '100%', style: { overflow: 'visible', pointerEvents: 'all' } }, closedFill, body)
  }
}
