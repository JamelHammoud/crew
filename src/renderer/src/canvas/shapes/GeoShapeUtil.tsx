...
const POLYGONS: Partial<Record<GeoKind, readonly [number, number][]>> = {
  ...
  octagon: [[0.293, 0], [0.707, 0], [1, 0.293], [1, 0.707], [0.707, 1], [0.293, 1], [0, 0.707], [0, 0.293]],
  star: [[0.5, 0], [0.612, 0.346], [0.976, 0.346], [0.682, 0.559], [0.794, 0.905], [0.5, 0.691], [0.206, 0.905], [0.318, 0.559], [0.024, 0.346], [0.388, 0.346]],
  rhombus: [[0.2, 0], [1, 0], [0.8, 1], [0, 1]],
  'rhombus-2': [[0, 0], [0.8, 0], [1, 1], [0.2, 1]],
  trapezoid: [[0.2, 0], [0.8, 0], [1, 1], [0, 1]],
  'arrow-right': [[0, 0.25], [0.6, 0.25], [0.6, 0], [1, 0.5], [0.6, 1], [0.6, 0.75], [0, 0.75]],
  'arrow-left': [[1, 0.25], [0.4, 0.25], [0.4, 0], [0, 0.5], [0.4, 1], [0.4, 0.75], [1, 0.75]],
  'arrow-up': [[0.25, 1], [0.25, 0.4], [0, 0.4], [0.5, 0], [1, 0.4], [0.75, 0.4], [0.75, 1]],
  'arrow-down': [[0.25, 0], [0.25, 0.6], [0, 0.6], [0.5, 1], [1, 0.6], [0.75, 0.6], [0.75, 0]],
  'x-box': [[0, 0], [1, 0], [1, 1], [0, 1]],
  'check-box': [[0, 0], [1, 0], [1, 1], [0, 1]]
}
...
export class GeoShapeUtil extends BaseBoxShapeUtil<GeoShape> {
  ...
  component(shape: GeoShape): ReactNode {
    ...
    const label = text ? createElement('div', { className: 'crew-rich-text', style: { position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: props.align.startsWith('start') ? 'flex-start' : props.align.startsWith('end') ? 'flex-end' : 'center', justifyContent: props.verticalAlign === 'start' ? 'flex-start' : props.verticalAlign === 'end' ? 'flex-end' : 'center', padding: 8, color: shapeColor(this.editor, props.labelColor), fontFamily: FONT_FAMILIES[props.font], fontSize: LABEL_FONT_SIZES[props.size] * props.scale, whiteSpace: 'pre-wrap', textAlign: props.align.startsWith('end') ? 'right' : props.align.startsWith('middle') ? 'center' : 'left', pointerEvents: 'all', visibility: editing ? 'hidden' : undefined }, dangerouslySetInnerHTML: { __html: richTextToHtml(props.richText as RichTextDocument) } }) : null
