...
import { noteShapeProps, type TLShape } from '../schema'
import { ShapeUtil } from './ShapeUtil'
import { COLORS, FONT_FAMILIES, FONT_SIZES, boxPath, plainText, richText, shapeElement } from './shared'

export type TLNoteShape = TLShape<'note'>
...
export class NoteShapeUtil extends ShapeUtil<TLNoteShape> {
  ...
  override getText(shape: TLNoteShape): string { return plainText(shape.props.richText) }
  ...
  component(shape: TLNoteShape): ReactNode {
    const { props } = shape
    const width = 200 * props.scale
    const height = (200 + props.growY) * props.scale
    const background = COLORS[props.color]
    const text = plainText(props.richText)
    return createElement('div', { style: { position: 'relative', width, height } }, shapeElement(boxPath(width, height), { color: props.color, fill: `${background}26`, width: 1 }), createElement('div', { style: { position: 'absolute', inset: 12 * props.scale, display: 'flex', alignItems: props.verticalAlign === 'start' ? 'flex-start' : props.verticalAlign === 'end' ? 'flex-end' : 'center', justifyContent: props.align.startsWith('start') ? 'flex-start' : props.align.startsWith('end') ? 'flex-end' : 'center', color: COLORS[props.labelColor], fontFamily: FONT_FAMILIES[props.font], fontSize: FONT_SIZES[props.size] * (props.fontSizeAdjustment ?? 1) * props.scale, lineHeight: 1.25, textAlign: props.align.startsWith('end') ? 'right' : props.align.startsWith('middle') ? 'center' : 'left', whiteSpace: 'pre-wrap', overflow: 'hidden', pointerEvents: 'all' } }, text))
  }
}
