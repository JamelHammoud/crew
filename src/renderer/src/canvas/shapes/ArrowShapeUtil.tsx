...
import { BindingUtil, ShapeUtil, type CrewShapePartial, type ShapeEditor, type ShapeHandle, type ShapeHandleDragInfo } from './ShapeUtil'
import { FONT_FAMILIES, FONT_SIZES, STROKES, plainText, richText } from './shared'
...
function bindingPoint(editor: ShapeEditor, binding: ArrowBinding, opposite: Vec, hasArrowhead: boolean, scale: number, strokeWidth: number): Vec | null {
  const target = editor.getShape?.(binding.toId)
  if (!target) return null
  const bounds = editor.getShapePageBounds?.(target)
  if (!bounds) return null
  const arrow = editor.getShape?.(binding.fromId)
  const anchor = new Vec(bounds.x + bounds.w * binding.props.normalizedAnchor.x, bounds.y + bounds.h * binding.props.normalizedAnchor.y)
  ...
  if (!binding.props.isExact) {
    ...
    const distances = [
      direction.x < 0 ? (bounds.x - anchor.x) / direction.x : direction.x > 0 ? (bounds.x + bounds.w - anchor.x) / direction.x : Infinity,
      direction.y < 0 ? (bounds.y - anchor.y) / direction.y : direction.y > 0 ? (bounds.y + bounds.h - anchor.y) / direction.y : Infinity
...
export function getArrowBindings(editor: ShapeEditor, arrow: ArrowShape): { start?: ArrowBinding; end?: ArrowBinding } {
  const bindings = (editor.getBindingsFromShape?.(arrow.id, 'arrow') ?? []).filter((binding): binding is ArrowBinding => binding.type === 'arrow')
  return {
...
export function getArrowTerminals(editor: ShapeEditor, arrow: ArrowShape): ArrowTerminals {
  ...
  return {
    start: bindings.start ? bindingPoint(editor, bindings.start, storedEnd, arrow.props.arrowheadStart !== 'none', arrow.props.scale, strokeWidth) ?? storedStart : storedStart,
    end: bindings.end ? bindingPoint(editor, bindings.end, storedStart, arrow.props.arrowheadEnd !== 'none', arrow.props.scale, strokeWidth) ?? storedEnd : storedEnd
  }
}
...
function arrowheadPath(point: Vec, toward: Vec, strokeWidth: number, kind: ArrowShape['props']['arrowheadEnd']): string | undefined {
...
export class ArrowShapeUtil extends ShapeUtil<ArrowShape> {
  ...
  override options = { hoverPreciseTimeout: 600, pointingPreciseTimeout: 320, showTextOutline: true, getCustomDisplayValues: () => ({}) }

  getDefaultProps(): ArrowShape['props'] { return { kind: 'arc', elbowMidPoint: 0.5, dash: 'draw', size: 'm', fill: 'none', color: 'black', labelColor: 'black', bend: 0, start: { x: 0, y: 0 }, end: { x: 2, y: 0 }, arrowheadStart: 'none', arrowheadEnd: 'arrow', richText: richText(), labelPosition: 0.5, font: 'draw', scale: 1 } }
  override canEdit(): boolean { return true }
  override canResize(): boolean { return false }
  override getText(shape: ArrowShape): string { return plainText(shape.props.richText) }
  getGeometry(shape: ArrowShape): Geometry2d { return arrowGeometry(this.editor, shape) }
  override getHandles(shape: ArrowShape): ShapeHandle[] {
    const { start, end } = getArrowTerminals(this.editor, shape)
    const middle = shape.props.kind === 'elbow'
  ...
  override onHandleDrag(shape: ArrowShape, info: ShapeHandleDragInfo<ArrowShape>): CrewShapePartial<ArrowShape> | void {
    ...
    if (shape.props.kind === 'elbow') {
      ...
      const midpoint = horizontal ? (info.handle.x - start.x) / Math.max(0.0001, end.x - start.x) : (info.handle.y - start.y) / Math.max(0.0001, end.y - start.y)
  ...
  component(shape: ArrowShape): ReactNode {
    ...
    const fillHead = (kind: ArrowShape['props']['arrowheadEnd']) => kind === 'triangle' || kind === 'square' || kind === 'dot' ? 'currentColor' : kind === 'diamond' || kind === 'inverted' ? canvasSurface(this.editor) : 'none'
    const editing = this.editor.getEditingShapeId?.() === shape.id
    return createElement('div', { style: { position: 'relative', width: '100%', height: '100%', color: shapeColor(this.editor, shape.props.color) } }, createElement('svg', { width: '100%', height: '100%', style: { overflow: 'visible', pointerEvents: 'all' } }, createElement('path', { d: body.toSimpleSvgPath(), fill: 'none', stroke: 'currentColor', strokeWidth, strokeDasharray: dash, strokeLinecap: 'round', strokeLinejoin: 'round' }), startHead && createElement('path', { d: startHead, fill: fillHead(shape.props.arrowheadStart), stroke: 'currentColor', strokeWidth, strokeLinecap: 'round', strokeLinejoin: 'round' }), endHead && createElement('path', { d: endHead, fill: fillHead(shape.props.arrowheadEnd), stroke: 'currentColor', strokeWidth, strokeLinecap: 'round', strokeLinejoin: 'round' })), text && createElement('div', { className: 'crew-rich-text', style: { position: 'absolute', left: middle.x, top: middle.y, transform: 'translate(-50%, -50%)', padding: '2px 5px', background: canvasSurface(this.editor), color: shapeColor(this.editor, shape.props.labelColor), fontFamily: FONT_FAMILIES[shape.props.font], fontSize: FONT_SIZES[shape.props.size], whiteSpace: 'pre-wrap', visibility: editing ? 'hidden' : undefined }, dangerouslySetInnerHTML: { __html: richTextToHtml(shape.props.richText as RichTextDocument) } }))
  }
}
...
export class ArrowBindingUtil extends BindingUtil<ArrowBinding> {
  ...
  getDefaultProps(): Partial<ArrowBinding['props']> { return { isPrecise: false, isExact: false, normalizedAnchor: { x: 0.5, y: 0.5 }, snap: 'none' } }
}
