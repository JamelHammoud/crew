...
function shorthand(font: TrimFont, size: number): string {
...
function readFace(font: TrimFont): FaceMetrics | null {
  ...
  try {
    context.font = shorthand(font, PROBE)
...
export function faceMetrics(font: TrimFont): FaceMetrics {
  const key = shorthand(font, PROBE)
  const known = faces.get(key)
  if (known) return known
  const face = readFace(font) ?? FALLBACK_FACE
  faces.set(key, face)
  return face
}
