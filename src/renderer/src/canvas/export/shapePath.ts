...
export function nodePath(kind: unknown, w: number, h: number, radius: unknown): string {
  if (kind === 'ellipse') return `M0 ${round(h / 2)} A${round(w / 2)} ${round(h / 2)} 0 1 0 ${round(w)} ${round(h / 2)} A${round(w / 2)} ${round(h / 2)} 0 1 0 0 ${round(h / 2)} Z`
  if (kind === 'triangle') return `M${round(w / 2)} 0 L${round(w)} ${round(h)} L0 ${round(h)} Z`
  if (kind === 'diamond') return `M${round(w / 2)} 0 L${round(w)} ${round(h / 2)} L${round(w / 2)} ${round(h)} L0 ${round(h / 2)} Z`
...
export function geoPath(kind: unknown, w: number, h: number, strokeWidth = 1): string {
  ...
  if (kind === 'arrow-right' || kind === 'arrow-left') {
    ...
    return kind === 'arrow-right' ? right : `M${round(w)} ${round(inset)} H${round(offset)} V0 L0 ${round(h / 2)} L${round(offset)} ${round(h)} V${round(h - inset)} H${round(w)} Z`
  }
  if (kind === 'arrow-up' || kind === 'arrow-down') {
    ...
    return kind === 'arrow-up' ? up : `M${round(inset)} 0 H${round(w - inset)} V${round(h - offset)} H${round(w)} L${round(w / 2)} ${round(h)} L0 ${round(h - offset)} H${round(inset)} Z`
  }
  if (kind === 'heart') return `M${round(w / 2)} ${round(h)} C0 ${round(h * 0.55)} 0 ${round(h * 0.2)} ${round(w * 0.25)} ${round(h * 0.15)} C${round(w * 0.42)} ${round(h * 0.1)} ${round(w * 0.5)} ${round(h * 0.25)} ${round(w / 2)} ${round(h * 0.32)} C${round(w * 0.5)} ${round(h * 0.25)} ${round(w * 0.58)} ${round(h * 0.1)} ${round(w * 0.75)} ${round(h * 0.15)} C${round(w)} ${round(h * 0.2)} ${round(w)} ${round(h * 0.55)} ${round(w / 2)} ${round(h)} Z`
  if (kind === 'cloud') return `M${round(w * 0.2)} ${round(h * 0.75)} C${round(w * 0.03)} ${round(h * 0.75)} 0 ${round(h * 0.5)} ${round(w * 0.14)} ${round(h * 0.4)} C${round(w * 0.13)} ${round(h * 0.16)} ${round(w * 0.38)} ${round(h * 0.05)} ${round(w * 0.52)} ${round(h * 0.24)} C${round(w * 0.68)} ${round(h * 0.08)} ${round(w * 0.9)} ${round(h * 0.2)} ${round(w * 0.87)} ${round(h * 0.43)} C${round(w * 1.03)} ${round(h * 0.5)} ${round(w * 0.97)} ${round(h * 0.75)} ${round(w * 0.8)} ${round(h * 0.75)} Z`
  if (kind === 'x-box') {
