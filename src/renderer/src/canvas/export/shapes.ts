...
function renderNote(shape: ExportShape, ctx: ShapeRenderContext): ShapeBody {
  ...
  if (!ctx.darkMode && !ctx.defs.has(id)) ctx.defs.set(id, `<filter id="${id}" x="-20%" y="-20%" width="140%" height="160%"><feDropShadow dx="0" dy="3" stdDeviation="4" flood-color="#000000" flood-opacity="0.25"/></filter>`)
...
function renderFrame(shape: ExportShape): ShapeBody {
  ...
  const background = typeof shape.meta?.background === 'string' && /^#[0-9a-f]{3,8}$/i.test(shape.meta.background)
  ...
  return { body, bounds: { x: 0, y: -28, w, h: h + 28 }, clip: `<rect width="${round(w)}" height="${round(h)}"/>`, mask: false }
}

function arrowMarker(ctx: ShapeRenderContext, shape: ExportShape, end: 'start' | 'end', kind: unknown, color: string, width: number): string {
  if (kind === 'none' || !kind) return ''
  const id = `arrow-${end}-${cleanId(shape.id)}`
  if (!ctx.defs.has(id)) {
    const closed = kind === 'dot'
    ...
    ctx.defs.set(id, `<marker id="${id}" markerWidth="10" markerHeight="10" refX="${end === 'end' ? 9 : 1}" refY="5" orient="auto-start-reverse" markerUnits="userSpaceOnUse"><g fill="${escapeXml(color)}" stroke="${escapeXml(color)}" stroke-width="${round(Math.max(1, width * 0.65))}" stroke-linejoin="round">${closed}</g></marker>`)
  }
...
function renderArrow(shape: ExportShape, ctx: ShapeRenderContext): ShapeBody {
  ...
  const labelPoint = curved
    ? { x: (1 - labelAt) ** 2 * start.x + 2 * (1 - labelAt) * labelAt * control.x + labelAt ** 2 * end.x, y: (1 - labelAt) ** 2 * start.y + 2 * (1 - labelAt) * labelAt * control.y + labelAt ** 2 * end.y }
    : { x: start.x + (end.x - start.x) * labelAt, y: start.y + (end.y - start.y) * labelAt }
  const lines = richLines(props.richText)
  const label = svgText({ x: labelPoint.x - 100, y: labelPoint.y - 20, w: 200, h: 40, lines, size: 20, lineHeight: 1.2, family: stringProp(props, 'font', 'sans'), color: themeColor(props.labelColor, 'solid', ctx.darkMode), align: 'middle', vertical: 'middle' })
  const points = curved ? [start, control, end] : [start, end]
  const bounds = pointsBounds(points, Math.max(12, width * 3))
  return { body: `<g transform="scale(${round(scale)})"><path d="${path}" fill="none" ${strokeAttrs(color, width, props.dash)} ${markers}/>${label}</g>`, bounds: bounds ? { x: bounds.x * scale, y: bounds.y * scale, w: bounds.w * scale, h: bounds.h * scale } : null, clip: null, mask: false }
}

function linePoints(props: Record<string, unknown>): Point[] {
  const records = props.points && typeof props.points === 'object' ? Object.values(props.points as Record<string, unknown>) : []
  return records
    .filter(point => point && typeof point === 'object')
    .sort((a, b) => String((a as { index?: unknown }).index ?? '').localeCompare(String((b as { index?: unknown }).index ?? '')))
...
function renderLine(shape: ExportShape, ctx: ShapeRenderContext): ShapeBody {
  ...
  const width = sizeValue(WEIGHTS, props.size, 'm')
  const path = props.spline === 'cubic' && points.length > 2
    ? smoothPath(points)
    : pointsPath(points, false)
  ...
  return { body, bounds: bounds ? { x: bounds.x * scale, y: bounds.y * scale, w: bounds.w * scale, h: bounds.h * scale } : null, clip: null, mask: false }
}
...
function smoothPath(points: Point[]): string {
  ...
  for (let index = 1; index < points.length; index++) {
    ...
    path += index === 1 ? ` L${round(mid.x)} ${round(mid.y)}` : ` Q${round(from.x)} ${round(from.y)} ${round(mid.x)} ${round(mid.y)}`
...
function decodedSegments(props: Record<string, unknown>): Point[] {
  ...
  for (const segment of props.segments) {
    ...
    const found = decodeDrawPoints(String((segment as { path?: unknown }).path ?? ''), (segment as { dim?: unknown }).dim)
    for (const point of found) points.push({ x: point.x * finite(props.scaleX, 1), y: point.y * finite(props.scaleY, 1) })
  }
  return points
}
...
function renderStrokeShape(shape: ExportShape, ctx: ShapeRenderContext, highlight: boolean): ShapeBody {
  ...
  const color = highlight ? themeColor(props.color, 'solid', ctx.darkMode) : themeColor(props.color, 'solid', ctx.darkMode)
  ...
  const bounds = pointsBounds(points, width)
  return { body: `<g transform="scale(${round(scale)})">${path}</g>`, bounds: bounds ? { x: bounds.x * scale, y: bounds.y * scale, w: bounds.w * scale, h: bounds.h * scale } : null, clip: null, mask: false }
}
...
function nodeFill(ctx: ShapeRenderContext, shape: ExportShape, paint: Paint, index: number): string {
  ...
  if (!ctx.defs.has(id)) {
    const stops = paint.stops.slice().sort((a, b) => a.at - b.at).map(stop => `<stop offset="${round(stop.at * 100)}%" stop-color="${escapeXml(stop.color)}" stop-opacity="${round(paint.opacity)}"/>`).join('')
    if (paint.type === 'radial') ctx.defs.set(id, `<radialGradient id="${id}">${stops}</radialGradient>`)
      ...
      ctx.defs.set(id, `<linearGradient id="${id}" x1="${round(50 - x)}%" y1="${round(50 - y)}%" x2="${round(50 + x)}%" y2="${round(50 + y)}%">${stops}</linearGradient>`)
    }
  }
  return `url(#${id})`
}
...
function nodeFilter(ctx: ShapeRenderContext, shape: ExportShape, effects: Effect[]): { attr: string; spill: number } {
  ...
  for (const effect of visible) {
    if (effect.type === 'shadow') {
      ...
      parts.push(`<feDropShadow dx="${round(effect.x)}" dy="${round(effect.y)}" stdDeviation="${round(effect.blur / 2)}" flood-color="${escapeXml(effect.color)}"/>`)
...
function renderDesignNode(shape: ExportShape, ctx: ShapeRenderContext): ShapeBody {
  ...
  const fills = (Array.isArray(props.fills) ? props.fills : []).map((paint, index) => ({ paint, index })).filter(({ paint }) => paint.visible).reverse().map(({ paint, index }) => `<path d="${path}" fill="${nodeFill(ctx, shape, paint, index)}" fill-opacity="${paint.type === 'solid' ? round(paint.opacity) : 1}"/>`).join('')
  ...
  const text = svgText({ x: 0, y: 0, w, h, lines: plainLines(props.text), size: finite(type.size, 14), lineHeight: finite(type.lineHeight, 1.5), family: type.family ?? 'sans', weight: type.weight, color: type.color ?? '#ffffff', align: type.align, vertical: type.vertical, italic: type.italic, spacing: type.spacing, decoration: type.decoration, transform: type.transform })
  const body = props.mask ? '' : `<g${filter.attr}${style}>${fills}${strokes}${text}</g>`
  const outside = (Array.isArray(props.strokes) ? props.strokes : []).filter(stroke => stroke.visible && stroke.align === 'outside').reduce((max, stroke) => Math.max(max, stroke.weight), 0)
  return { body, bounds: props.mask ? null : shapeBox(w, h, Math.max(filter.spill, outside)), clip: props.clip || props.mask ? `<path d="${path}"/>` : null, mask: props.mask === true }
}
...
function renderBookmark(shape: ExportShape, ctx: ShapeRenderContext): ShapeBody {
  ...
  const image = typeof asset?.props.image === 'string' && asset.props.image ? `<image href="${escapeXml(asset.props.image)}" width="${round(w)}" height="${round(h * 0.55)}" preserveAspectRatio="xMidYMid slice"/>` : ''
  return { body: `<rect width="${round(w)}" height="${round(h)}" rx="8" fill="#ffffff" stroke="#d4d4d4"/>${image}${svgText({ x: 16, y: h * 0.58, w: w - 32, h: h * 0.17, lines: [title], size: 18, lineHeight: 1.2, family: 'sans', weight: 600, color: '#1d1d1d' })}${svgText({ x: 16, y: h * 0.76, w: w - 32, h: h * 0.18, lines: [description], size: 13, lineHeight: 1.3, family: 'sans', color: '#666666' })}`, bounds: shapeBox(w, h), clip: null, mask: false }
}
...
function renderEmbed(shape: ExportShape): ShapeBody {
  ...
  return { body: `<rect width="${round(w)}" height="${round(h)}" rx="8" fill="#f5f5f5" stroke="#b8b8b8"/><text x="${round(w / 2)}" y="${round(h / 2)}" text-anchor="middle" fill="#555555" font-family="ui-sans-serif,system-ui,sans-serif" font-size="14">${escapeXml(url)}</text>`, bounds: shapeBox(w, h), clip: null, mask: false }
}
