  ...
  return {
    ...
    openUrl: (url) => {
      ...
      if (active && active.kind === "web" && !active.initialUrl) {
        set((s) => ({
          tabs: s.tabs.map((t) =>
            t.id === active.id ? { ...t, initialUrl: url, url } : t,
          ),
        }));
        return;
      }
    ...
    openImage: (src, name) => {
      const existing = get().tabs.find(
        (t) => t.kind === "image" && t.initialUrl === src,
      );
      if (existing) {
        set({ activeTabId: existing.id });
        return;
    ...
    openFile: (path, line = null, diff = null) => {
      ...
      if (existing) {
        set((s) => ({
          ...
          tabs: s.tabs.map((t) =>
            t.id === existing.id
              ? { ...t, line, diff, generation: t.generation + 1 }
              : t,
          ),
        }));
        return;
      ...
      if (active && active.kind === "web" && !active.initialUrl) {
        set((s) => ({
          tabs: s.tabs.map((t) =>
            t.id === active.id
              ? { ...t, kind: "file" as const, path, line, diff }
              : t,
          ),
        }));
        return;
    ...
    openFiles: () => {
      ...
      const active = tabs.find((t) => t.id === activeTabId);
      const existing =
        active?.kind === "file" ? active : tabs.find((t) => t.kind === "file");
      if (existing) {
        set((s) => ({
          activeTabId: existing.id,
          tabs: s.tabs.map((t) =>
            t.id === existing.id
              ? { ...t, tree: true, open: reveal(t.open, t.path) }
              : t,
          ),
        }));
        return;
    ...
    openMusic: () => {
      ...
      if (active && active.kind === "web" && !active.initialUrl) {
        set((s) => ({
          tabs: s.tabs.map((t) =>
            t.id === active.id ? { ...t, kind: "music" as const } : t,
          ),
        }));
        return;
      }
    ...
    openGame: () => {
      ...
      if (active && active.kind === "web" && !active.initialUrl) {
        set((s) => ({
          tabs: s.tabs.map((t) =>
            t.id === active.id ? { ...t, kind: "game" as const } : t,
          ),
        }));
        return;
      }
    ...
    openAside: (threadId, title) => {
      const existing = get().tabs.find(
        (t) => t.kind === "aside" && t.threadId === threadId,
      );
      if (existing) {
        set({ activeTabId: existing.id });
        return;
    ...
    openSubagent: (threadId, parentThreadId) => {
      const existing = get().tabs.find(
        (t) => t.kind === "agent" && t.parentThreadId === parentThreadId,
      );
      if (existing) {
        set((s) => ({
          activeTabId: existing.id,
          tabs: s.tabs.map((t) =>
            t.id === existing.id ? { ...t, threadId } : t,
          ),
        }));
        return;
      }
    ...
    showPlan: (threadId) => {
      ...
      set((s) => ({
        tabs: [tab, ...s.tabs.filter((t) => t.kind !== "plan")],
        activeTabId: tab.id,
        closedPlans,
      }));
    },
    ...
    showWork: (threadId) => {
      ...
      set((s) => ({
        tabs: [tab, ...s.tabs.filter((t) => t.kind !== "work")],
        activeTabId: tab.id,
        closedBoards,
      }));
    },
        ...
        tabs: s.tabs.map((t) =>
          t.id === id
            ? {
                ...t,
                tree: !t.tree,
                open: t.tree ? t.open : reveal(t.open, t.path),
              }
            : t,
        ),
      })),
      ...
      set((s) => ({
        tabs: s.tabs.map((t) =>
          t.id === id
            ? {
                ...t,
                open: t.open.includes(path)
                  ? t.open.filter((one) => one !== path)
                  : [...t.open, path],
              }
            : t,
        ),
    ...
    navigateTab: (id, url) =>
      set((s) => ({
        tabs: s.tabs.map((t) =>
          t.id === id ? { ...t, initialUrl: url, url } : t,
        ),
      })),
    ...
    reloadTab: (id) =>
      set((s) => ({
        tabs: s.tabs.map((t) =>
          t.id === id ? { ...t, generation: t.generation + 1 } : t,
        ),
      })),
    updateTab: (id, patch) =>
      set((s) => ({
