...
const ACTIVITY_TRANSITION_MS = 240
...
function useActivityPerformance(activity: AgentActivity): ActivityPerformance {
  const [performance, setPerformance] = useState<ActivityPerformance>({
    current: activity,
    changing: false,
    turn: 0
  })
...
export default function AgentIcon({
  ...
  const style = {
    ...
    '--agent-transition-blur': `${Math.max(0.55, box * 0.018)}px`
  } as CSSProperties
