...
- One shell is always kept ready, so a tab opens on a prompt instead of a blinking cursor. A login shell reads the whole profile before it says anything, which is over three seconds cold on a machine that loads a version manager, and none of that wait belongs to the person who just asked for a terminal. `warm` starts the spare, `claim` hands it over, and every `open` starts the next one. The panel asks for one when it appears and again when the New menu opens, so the spare is already up by the time anyone picks Terminal.
