...
- `Skeleton` is what stands in for content that has not arrived. One shimmer, and every instance is pinned to the same start time the way `Spinner` is, so a stack of them breathes together instead of rippling.
