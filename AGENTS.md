...
- A build run from source wears a blueprint of the same three discs: ruled paper, and the discs lit like glass, blue on near black in dark and white on blue in light. It is the one place in the project that is blue, so a dev window is never mistaken for the installed app sitting beside it in the dock. Each disc is shaded by one set of gradients in bounding box units, so all three catch the light the same way: a body lit from the upper left, light coming back along the bottom edge, one specular near the top, and a shadow cast into the gap behind.
